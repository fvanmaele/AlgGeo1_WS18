...
aufgefasst werden. Für \emph{homogene}\textbf{ }Polynome $f_{1},\ldots,f_{n}\in k[X_{0},\ldots X_{n}]$
 (nicht notwendig vom selben Grad) können wir demnoch die Verschwindungsmengen
definieren:
\[
V_{+}(f_{1},\ldots,f_{n})=\{(x_{0}:\cdots:x_{n})\in\mathbb{P}^{n}\mid f_{j}(x_{0},\ldots,x_{n})=0\ \forall j\}
\]
...
$\Rightarrow0\,\cancel{\equiv}\,\sum_{d}g_{d}(x_{0},\ldots,x_{n})T^{d}\in k[T]$
...
\end{enumerate}
$\Rightarrow$ (Verkleben) $g\circ\phi\in\mathcal{O}_{V}(\phi^{-1}(V))$.\selectlanguage{ngerman}%
\end{itemize}
\end{proof}
