...
\section{Funktionenkörper einer Prävarietät}\label{sec:funktionenkorper-praevarietaet}
\begin{defn}[orig. 43]\label{def:funktionenkoerper-praevarietaet}
...
\end{proof}
\begin{rem}[orig. 44]\label{rem:funktionenkoerper-nicht-funktoriell}
...
\[
\xymatrix{\mathcal{O}_{X}(W)\ar@{^{(}->}[r] & K(W)\ k\text{-Algebren}\\
\mathcal{O}_{X}(U)\ar@{^{(}->}[u]
}
\]
\item[(iii)] TODO.
\end{proof}
