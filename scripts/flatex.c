 ...
 * Static prototypes.
\*======================================================================*/
static void flatIt(FILE *flOut, char *szInName, int level, structFlags *pFlags);
...
static void handleIncludeCommand(char *line, char *lpszInclude, FILE *flOut,
                                 int level, structFlags *pFlags) {
...
static FILE *fopenTex(char *file, char *mode) {
  FILE *fl;
...
static void flatIt(FILE *flOut, char *pSzInName, int level,
                   structFlags *pFlags) {
  FILE *flIn;
  ...
  if (!pFlags->fQuiet)
    fprintf(flOut, "%%%cflatex input: [%s]\n",
            pFlags->cSpecialInputLevel > 0 ? '*' : ' ', szInName);
  if (pFlags->verbose) {
    printf("\t");
    spacesByLevel(level);
    printf("%s\n", szInName);
  ...
  *lpszRemark = 0;
  while (!feof(flIn)) {
    ...
    do {
      ...
      if (pFlags->fBibInsert && (lpszBib != NULL || lpszBibStyle != NULL)) {
        lpszName = (char *)myMalloc(LINE_SIZE);
      ...
      do {
        ...
        if (is_str_prefix(lpszInclude, "\\includeversion") ||
            is_str_prefix(lpszInclude, "\\includegraphics")) {
          repFlag = 1;
          inc = lpszInclude + 1;
          continue;
        }
      ...
      if (lpszInclude != NULL) {
        ...
        if (lpszNewCommand == NULL) {
          handleIncludeCommand(line, lpszInclude, flOut, level, pFlags);
          cont = 1;
          fInclude = TRUE;
        }
      }
...
static void replaceExt(char *str, char *ext) {
  ...
  len = strlen(str);
  ind = len - 1;
  while (ind >= 0 && str[ind] != '.' && str[ind] != '\\' && str[ind] != '/')
    ind--;
...
static char strCmpPrefixAndCopy(char *line, char *str, char *outName) {
  char *pos, *pPreLine;
...
static void writeFile(FILE *flIn, char *pOutName, int level) {
  FILE *flOut;
  char *lpszLine;
...
static void flatOutFile(char *fileName, structFlags *pFlags) {
  FILE *flIn;
...
static void flatFile(char *fileName, structFlags *pFlags) {
  ...
  if (inLen < 4 ||
      (szInName[inLen] != '.' && strcmp(szInName + inLen - 4, ".tex") != 0)) {
    strcat(szInName, ".tex");
  }
...
static char isFlag(char *str, char ch) {
  if (str[0] == '-' && (str[1] == ch || str[1] == toupper(ch)) && (str[2] == 0))
    return TRUE;
